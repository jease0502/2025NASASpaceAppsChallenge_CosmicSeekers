...
function updatePredictionSummary() { if (!DOMElements.predictionDetails || !DOMElements.predictionSummary) return; const predictions = DOMElements.predictionDetails.children; let confirmedCount = 0; let fpCount = 0; for (const pred of predictions) { if (pred.querySelector('.text-green-400')) confirmedCount++; else if (pred.querySelector('.text-red-400')) fpCount++; } const total = confirmedCount + fpCount; if (total === 0) { DOMElements.predictionSummary.innerHTML = '<p>Click on a planet to run AI prediction.</p>'; return; } DOMElements.predictionSummary.innerHTML = ` <p class="flex justify-between"><strong>Analyzed Objects:</strong> <span>${total}</span></p> <p class="flex justify-between"><strong>Confirmed Planets:</strong> <span class="text-green-400">${confirmedCount}</span></p> <p class="flex justify-between"><strong>False Positives:</strong> <span class="text-red-400">${fpCount}</span></p> `; }
async function addPredictionToCard(planetData, systemData) { if (!systemData) return; const planetName = `${systemData.star.system_name}.${planetData.planet_index.toString().padStart(2,'0')}`; if (document.getElementById(`pred-${planetName}`) || !DOMElements.predictionDetails) return; const predColor = planetData.model_prediction === 'CONFIRMED' ? 'text-green-400' : 'text-red-400'; const predictionEntry = document.createElement('div'); predictionEntry.id = `pred-${planetName}`; predictionEntry.className = 'grid grid-cols-3 gap-2 items-center'; predictionEntry.innerHTML = `<span>${planetName}</span><span class="${predColor} font-bold">${planetData.model_prediction}</span><span class="text-right">${planetData.model_confidence}%</span>`; DOMElements.predictionDetails.appendChild(predictionEntry); updatePredictionSummary(); if (DOMElements.predictionCard) DOMElements.predictionCard.classList.add('visible'); }
function createModelSelectorHTML() { let options = ''; const defaultModel = AppState.latestTrainedModelName || Object.keys(availableModels)[0]; for (const [key, name] of Object.entries(availableModels)) { const isLatestTrained = AppState.latestTrainedModelName === key; const displayName = isLatestTrained ? `${name} (Latest Trained)` : name; const selected = key === defaultModel ? 'selected' : ''; options += `<option value="${key}" ${selected}>${displayName}</option>`; } return ` <div class="flex flex-col gap-2 mt-3"> <select id="info-panel-model-selector" class="bg-gray-700 border border-gray-600 rounded-md text-xs p-2 w-full"> ${options} </select> <button id="predict-btn" class="predict-button w-full"> <span class="material-symbols-outlined">psychology</span> <span>AI Predict</span> </button> </div> `; }
...
export const MainExplorer = {
    init(systemsData, onStateChange) {
        ...
        async function predictSinglePlanet(planetData, planetSystem) {
            const selector = document.getElementById('info-panel-model-selector'); const button = document.getElementById('predict-btn'); if (!selector || !button) return; const selectedModel = selector.value; button.textContent = 'Predicting...'; button.disabled = true; selector.disabled = true; const useLatest = AppState.latestTrainedModelName === selectedModel; if (useLatest) { console.log(`Requesting latest temp model: ${selectedModel}`); } const result = await ApiService.predictSystemData(selectedModel, [planetData], useLatest); const activeSystemData = systemsData[AppState.activeSystemIndex]; if (result && !result.error) { const predictionResult = result[0]; planetData.model_prediction = predictionResult.prediction; planetData.model_confidence = (predictionResult.confidence * 100).toFixed(0); addPredictionToCard(planetData, activeSystemData); if (planetData.model_prediction === 'FALSE POSITIVE') { ThreeManager.explodePlanet(planetSystem.planetGroup); setTimeout(() => UIManager.hideInfoPanel(), 500); } const resultDiv = document.createElement('div'); const resultColor = planetData.model_prediction === 'CONFIRMED' ? 'text-green-400' : 'text-red-400'; resultDiv.className = `font-bold mt-3 p-2 rounded-lg text-center ${resultColor === 'text-green-400' ? 'bg-green-500/20' : 'bg-red-500/20'}`; resultDiv.innerHTML = `AI Result: <span class="${resultColor}">${planetData.model_prediction}</span>`; const container = button.parentElement; if(container) { container.innerHTML = ''; container.appendChild(resultDiv); } } else { button.textContent = 'Prediction Failed'; setTimeout(() => { if (button && selector) { button.innerHTML = `<span class="material-symbols-outlined">psychology</span><span>AI Predict</span>`; button.disabled = false; selector.disabled = false; } }, 3000); }
        }
