...
FINAL_COLUMNS_WITH_EXPLANATIONS = {
    ...
    'lightcurve_id': '光變曲線ID (KOI用kepid, TESS用tid)',
    'period': '軌道週期 (天)',
    'duration': '凌日持續時間 (小時)',
...
# --- 原始欄位與標準欄位的對應表 ---
KOI_COLUMN_MAP = {
    'kepoi_name': 'source_id', 'koi_disposition': 'disposition', 'kepid': 'lightcurve_id', 'koi_period': 'period',
...
TESS_COLUMN_MAP = {
    'toi': 'source_id', 'tfopwg_disp': 'disposition', 'tid': 'lightcurve_id', 'pl_orbper': 'period',
...
def standardize_dataset(df: pd.DataFrame, dataset_type: str, column_map: dict):
    ...
    # 修正 lightcurve_id 格式，移除 .0
    if 'lightcurve_id' in standardized_df.columns:
        standardized_df['lightcurve_id'] = standardized_df['lightcurve_id'].astype('Int64').astype(str)
        standardized_df['lightcurve_id'] = standardized_df['lightcurve_id'].replace('nan', np.nan)
...
# --- 主程式執行區塊 ---
base_path = '.'
koi_file_name = 'cumulative_2025.09.26_22.37.23.csv'
