...
STANDARD_COLUMNS = [
    ...
    'stellar_logg_err_lower', 'ra', 'dec', 'is_outlier'
]
KOI_COLUMN_MAP = {
...
def remove_outliers_from_columns(df: pd.DataFrame, columns_to_clean: list) -> pd.DataFrame:
    print("\n" + "="*50); print(f"--- 刪除指定欄位的離群值 ---"); print(f"目標欄位: {columns_to_clean}"); print("="*50 + "\n")
    initial_rows = len(df)
    indices_to_drop = set()
    ...
    if indices_to_drop:
        ...
        print(f"原始資料筆數: {initial_rows}"); print(f"清理後資料筆數: {len(df_cleaned)}"); print(f"刪除比例: {rows_dropped/initial_rows:.2%}")
    else:
        print("\n在指定欄位中未找到需要刪除的離群值。")
        df_cleaned = df.copy()
    print("\n" + "-"*50 + "\n"); return df_cleaned
...
def add_outlier_flag_column(df: pd.DataFrame) -> pd.DataFrame:
    print("\n" + "="*50); print("--- 為數據集新增離群值標記 ('is_outlier') 欄位 ---")
    ...
    print(f"標記完成！共找到 {num_outliers} 筆資料在至少一個欄位中為離群值。"); print("="*50 + "\n")
    return df

def find_and_rank_extremes(df: pd.DataFrame, top_n: int = 20):
    print("\n" + "="*60); print("--- 尋找極端值之最 (Top Outliers by Robust Z-score) ---"); print("="*60 + "\n")
    numeric_cols_to_check = [
        'period', 'duration', 'depth', 'planet_radius', 'equilibrium_temp',
        'insolation_flux', 'stellar_magnitude', 'stellar_temp',
        'stellar_radius', 'stellar_logg']
    ...
    print(f"報告：清理後數據集中最極端的 Top {top_n} 筆數據"); print("穩健 Z 分數越高，代表該值在它的屬性中越罕見、越極端。\n")
    print(final_report[['source_id', 'extreme_field', 'extreme_value', 'robust_z_score', 'median_value']].to_string(index=False)); print("-" * 60 + "\n")

def count_values_greater_than(df: pd.DataFrame, threshold: int = 5):
    print("\n" + "="*50); print(f"--- 統計數值大於 {threshold} 的資料筆數 ---"); print("="*50 + "\n")
    numeric_cols_to_check = [
        'period', 'duration', 'depth', 'planet_radius', 'equilibrium_temp',
        'insolation_flux', 'stellar_magnitude', 'stellar_temp',
        'stellar_radius', 'stellar_logg']
    for col in numeric_cols_to_check:
        data = df[col].dropna()
        if len(data) == 0: print(f"欄位 '{col}': 沒有有效數據。"); continue
...
# --- NEW: Master Pipeline Function (to be called by app.py) ---
def run_full_preprocessing_pipeline():
    """
    Runs the entire preprocessing pipeline and captures console output.
    This function will be imported and called by the Flask app.
    """
    old_stdout = sys.stdout
    sys.stdout = captured_output = io.StringIO()
    
    output_filename = 'static/processed_data/combined_exoplanet_data_cleaned.csv'
    
    try:
        koi_file_path = 'Data/cumulative_2025.09.26_19.48.09.csv'
        tess_file_path = 'Data/TOI_2025.09.26_19.47.35.csv'
        all_dfs = []
        ...
        if len(all_dfs) > 0:
            ...
            cols_to_keep = [col for col in STANDARD_COLUMNS if col != 'is_outlier' and col in combined_df.columns]
            ...
            cleaned_df = cleaned_df.reindex(columns=STANDARD_COLUMNS, fill_value=np.nan)
        ...
        sys.stdout = old_stdout # IMPORTANT: Always restore stdout

    # Return the final file path and the captured text log
    return output_filename, captured_output.getvalue()


# --- NEW: Master Pipeline for a SINGLE Uploaded File ---
def run_pipeline_on_single_file(dataset_type: str, df: pd.DataFrame, column_map: dict):
    """
    Takes a VALIDATED dataframe and runs the full cleaning and analysis pipeline on it.
    """
    old_stdout = sys.stdout
    sys.stdout = captured_output = io.StringIO()
    
    output_filename = f'static/processed_data/cleaned_{dataset_type.lower()}_data.csv'

    try:
        print(f"--- 開始處理已上傳的 {dataset_type} 資料 ---")
        
        # 1. Standardize
        standardized_df = standardize_dataset(df, dataset_type, column_map)
        print(f"資料標準化完成！總資料筆數: {standardized_df.shape[0]}")
